...
class RuleController extends AuthController{
	...
	public function admin_add(){
		$auth_group=M('auth_group')->where(array('id'=>array('neq',1)))->select();
	...
	public function admin_edit(){
		...
		$auth_group_access=M('auth_group_access')->where(array('uid'=>$admin_list['admin_id']))->getField('group_id'); //角色ID
		$branchlist = M('department_list')->where(array('is_del'=>1))->select();
	...
	public function admin_runedit(){
		$admin_list=M('admin');
		$data['admin_username']=I('admin_username');
		$admin_pwd =I('admin_pwd');
		$data['admin_realname']=I('admin_realname');
		$data['admin_tel']=I('admin_tel');
		$data['department_id']=I('department_id');
		$data['group_id']=I('group_id');
		$data['admin_open']=I('admin_open');
		$datat['admin_id']=I('admin_id');
		if($admin_pwd){
			$data['admin_pwd'] = $admin_pwd;
			$admin_pwd_salt=Stringnew::randString(10);
			$data['admin_pwd_salt']=$admin_pwd_salt;
			$data['admin_pwd']=encrypt_password($data['admin_pwd'],$admin_pwd_salt);
			$data['admin_changepwd']=time();
		}
		$admin_list->where(array('admin_id'=>$datat['admin_id']))->save($data);
		if($data['group_id']){
			$rst=M('auth_group_access')->where(array('uid'=>$datat['admin_id']))->find();
			if($rst){
				//修改
				$rst=M('auth_group_access')->where(array('uid'=>$datat['admin_id']))->setField('group_id',$data['group_id']);
			}else{
				//增加
				$datat['uid']=$datat['admin_id'];
				$datat['group_id']=$data['group_id'];
				$rst=M('auth_group_access')->add($datat);
			}
		}
		if($rst!==false){
	...
	public function adminEdit(){
		$admin_id=I('admin_id');
		$admin_pwdj=I('admin_pwdj');
		$admin_pwdt=I('admin_pwdt');
		$admin_pwd=I('admin_pwd');
		$data['admin_tel']=I('admin_tel');
		$info = M('admin')->where(array('admin_id'=>$admin_id))->find();
		if(encrypt_password($admin_pwdj,$info['admin_pwd_salt']) !== $info['admin_pwd']){
			$this->error('旧密码与原密码不一致',U('Admin/Sys/profile'),0);
		}
		if($admin_pwd !== $admin_pwdt){
			$this->error('两次输入的密码不一致',U('Admin/Sys/profile'),0);
		}
		if($admin_pwd){
			$admin_pwd_salt=Stringnew::randString(10);
			$data['admin_pwd_salt']=$admin_pwd_salt;
			$data['admin_pwd']=encrypt_password($admin_pwd,$admin_pwd_salt);
			$data['admin_changepwd']=time();
		}
		$rst = M('admin')->where(array('admin_id'=>$admin_id))->save($data);
		if($rst!==false){
			$this->success('修改成功',U('Admin/Sys/profile'),1);
		}else{
			$this->error('修改失败',U('aAdmin/Sys/profile'),0);
		}
	}
	//删除管理员
