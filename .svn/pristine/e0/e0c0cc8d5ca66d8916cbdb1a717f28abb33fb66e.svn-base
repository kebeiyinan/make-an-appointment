...
//	global  $keys;
//	global  $appsecret;
//	global  $curl_proxy_host;//"10.152.18.220";
//	global  $curl_proxy_port;//8080;
//	global  $report_levenl;
//	global  $curl_timeout;
/**
 * 所有接口的基类
 */
class Common_util_pub
{
	...
	public function getSign($Obj)
	{
		...
		//签名步骤二：在string后加入KEY
		$String = $String."&key=".$GLOBALS['keys'];
	...
	function postXmlSSLCurl($xml,$url,$second=30)
	{
		...
		curl_setopt($ch,CURLOPT_SSLCERT, getcwd().$GLOBALS['cert_pem']);
		//默认格式为PEM，可以注释
		curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
		curl_setopt($ch,CURLOPT_SSLKEY, getcwd().$GLOBALS['key_pem']);
		//post提交方式
		curl_setopt($ch,CURLOPT_POST, true);
		curl_setopt($ch,CURLOPT_POSTFIELDS,$xml);
		$data = curl_exec($ch);
...
class Wxpay_client_pub extends Common_util_pub 
{
	...
	function createXml()
	{
	   	$this->parameters["appid"] = $GLOBALS['appid'];//公众账号ID
	   	$this->parameters["mch_id"] = $GLOBALS['mchid'];//商户号
	...
	function postXml()
	{
		...
		$this->response = $this->postXmlCurl($xml,$this->url,$GLOBALS['curl_timeout']);
	...
	{	
	    $xml = $this->createXml();
		$this->response = $this->postXmlSSLCurl($xml,$this->url,$GLOBALS['curl_timeout']);
		return $this->response;
	}
...
class sendredpack_pub extends Wxpay_client_pub
{
   function __construct() {
	  ...
	  //设置curl超时时间
	  $this->curl_timeout = $GLOBALS['curl_timeout'];
	}
	/**
	* 生成接口参数xml
	*/
	function createXml()
	{
	  try
	 {
	  $this->parameters["mch_billno"] = $GLOBALS['mch_billno'];//商户订单号
	  $this->parameters["wxappid"] = $GLOBALS['appid'];//公众账号ID
	  $this->parameters["mch_id"] = $GLOBALS['mchid'];//商户号
	  $this->parameters["nonce_str"] = $this->createNoncestr();//随机字符串
	  $this->parameters["sign"] = $this->getSign($this->parameters);//签名
	  return $this->arrayToXml($this->parameters);
...
{	
	function __construct() 
	{
		...
		//设置curl超时时间
		$GLOBALS['curl_timeout'] = $GLOBALS['curl_timeout'];
	}
	...
	{
		//		var_dump($GLOBALS['appid']);
		//		var_dump($GLOBALS['appsecret']);
		$this->postXml();
		$this->result = $this->xmlToArray($this->response);
		$prepay = $this->result;
		return $prepay;
...
class NativeCall_pub extends Wxpay_server_pub
{
	...
	function createXml()
	{
		if($this->returnParameters["return_code"] == "SUCCESS"){
		   	$this->returnParameters["appid"] = $GLOBALS['appid'];//公众账号ID
		   	$this->returnParameters["mch_id"] = $GLOBALS['mchid'];//商户号
...
class JsApi_pub extends Common_util_pub
{
	...
	function getOpenid()
	{
        ...
        //初始化curl
       	$ch = curl_init();
		//设置超时
		curl_setopt($ch, CURLOP_TIMEOUT, $GLOBALS['curl_timeout']);
		curl_setopt($ch, CURLOPT_URL, $url);
        curl_setopt($ch,CURLOPT_SSL_VERIFYPEER,FALSE);
        curl_setopt($ch,CURLOPT_SSL_VERIFYHOST,FALSE);
		curl_setopt($ch, CURLOPT_HEADER, FALSE);
